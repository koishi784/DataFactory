 ...
 */
@Data
@Schema(description = "新增数据标准请求参数")
public class DataStandardCreateRequest {

    @Schema(description = "中文名称", example = "企业类型", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "中文名称不能为空")
    @Size(max = 50, message = "中文名称最大 50 字符")
    @Pattern(regexp = "^[a-zA-Z\u4e00-\u9fa5]+$", message = "中文名称仅支持中文及英文大小写")
    private String name;

    @Schema(description = "英文名称", example = "firmType", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "数据长度（仅 String 类型可填，须为正整数）", example = "50")
    private Integer length;

    @Schema(description = "精度（仅 Float 类型可填，须为非负整数）", example = "2")
    ...
    @Schema(description = "是否可为空：0=可为空 / 1=不可为空", example = "1", defaultValue = "0")
